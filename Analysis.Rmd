...
The analysis of the given data did not suffice to prove a relationship between 
the transmission type (automatic/manual) and the miles per gallon (MPG). The 
final model suggests an increase in MPG of 0.276 (+- 2.75), which is equal to a 
random noise with a 84.2% chance.
...
figure 1. Except for the _V/S_ field interpretation was provided there, for that
one a little investigation was necessary (see appendix).
...
In the following, a series of nested models will attempt to refine the initial
conclusion. When adjusting for further variables, the significance of the AM
variable disappears and no further refinement brings it back.
...
Including it and its second power further improves the models giving much 
smaller residuals (cca. -2 to +4) than before (cca. -4 to +6).
...
transmission cars with high MPG and manual ones with a low MPG.
...
Do I use dummies to get rid of the effect of the outliers?
No.

TODO: accomplish this with the last model

is.auto = mtcars$am == "automatic"
model.nested.auto.line = 
  model.nested$coefficients[1] + 
  model.nested$coefficients[3] * mtcars$wt[is.auto];
model.nested.manual.line =
  model.nested$coefficients[1] + 
  model.nested$coefficients[2] +
  model.nested$coefficients[3] * mtcars$wt[!is.auto];
lines(mtcars$wt[is.auto], model.nested.auto.line, col = "blue")
lines(mtcars$wt[!is.auto], model.nested.manual.line, col = "green")




#
# ammanual     -1.8621     1.4041  -1.326  0.19548  
# 
# the MPG of manual transmission cars seems lower than that of automatic ones.
# the difference is -1.8621, however, this can be down to the driving style of
# the test drivers. 

o.wt = order(mtcars$wt)
plot(mtcars$wt, mtcars$mpg)
lines(mtcars$wt[o.wt], fitted(model.nested.2)[o.wt])

# plot(mtcars$wt, mtcars$mpg)
# lines(mtcars$wt[o.wt], fitted(model.nested.3)[o.wt])





# we may try to remove the outliers
mtcars.2 = mtcars[-c(8, 18, 20), ]
# model.nested.2.noout = update(model.nested.2, data = mtcars.2)
#summary(model.nested.2.noout)
# plot(model.nested.2.noout)

# but there is at least one similarity, the low nr. of careburators
plot(mtcars$carb, resid(model.nested.2))

# beyond carb, the residual has a relatively high correlation with mtcars$qsec
# and mtcars$hp.
# actually it is reasonable to assume that the more horsepower the car has, the 
# shorter it takes to accelerate up on a quarter mile. 
# 
# mtcars$hp / mtcars$wt has a higher correlation with the residuals than qsec
# (highest individual covariate)

# print("Nested model #3")
# model.nested.3 = lm(mpg ~ am + wt + I(wt ^ 2) + I(hp / wt), data = mtcars)

print("Nested model #3")
model.nested.3 = lm(mpg ~ am + wt + I(wt ^ 2) + hp, data = mtcars)
summary(model.nested.4)

# model.nested.5 = lm(mpg ~ am + wt + I(wt ^ 2) + hp + I(hp ^ 2), data = mtcars)
# summary(model.nested.5)

model.nested.6 = lm(mpg ~ am + wt + I(wt ^ 2) + hp + I(hp ^ 2) + I(hp * wt), data = mtcars)

# plot(mtcars$mpg, col = "red")
# points(fitted(model.nested.6), col = "blue")
# 
# 

plot(mtcars$mpg, fitted(model.nested.6))
